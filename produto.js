...
(async function () {
  ...
  try {
    ...
    // se o preço cadastrado indicar indisponibilidade (ex: "INDISPONÍVEL", "vendido"),
    // desativa o botão de encomendar em vez de deixar o cliente preencher um pedido
    // que não pode ser atendido.
    const textoPreco = (p.preco || "").toLowerCase();
    const indisponivel = /indispon|vendid|esgotad/.test(textoPreco);
    if (indisponivel) {
      const btn = document.getElementById("btn-encomendar");
      btn.removeAttribute("href");
      btn.style.opacity = "0.5";
      btn.style.cursor = "default";
      btn.textContent = "Peça indisponível no momento";
